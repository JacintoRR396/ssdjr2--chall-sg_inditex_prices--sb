...
import com.ssdjr2.chall.sg.inditex_prices.factory.*;
...
class PriceDtoMapperTest {
	...
	void givenValidPriceSearchQueryDTO_whenMappingToPrice_thenReturnsPopulatedPriceDomain() {
		...
		assertThat(result.getApplicationDates().getStartDate()).isEqualTo(targetDate);
		assertThat(result.getApplicationDates().getEndDate()).isEqualTo(targetDate);
